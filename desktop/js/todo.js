        ...
        success: function (data) {
            ...
            if (data.result.length != 0) {
                ...
                for (var k = 0; k < data.result.length; k++) {
                    var time_class = '';
                    if (
                        data.result[k].configuration.cron_todo &&
                        data.result[k].configuration.cron_todo != ''
                    ) {
                    ...
                    if (!data.result[k].configuration.type) {
                        if (data.result[k].isVisible == 1) {
                            ...
                            html +=
                                '<li id="' +
                                data.result[k].id +
                                '" class="list-group-item list_edit" style="background-color:transparent;font-size : 1.1em;padding:10px;"><span><input value="' +
                                data.result[k].id +
                                '" type="checkbox" ></span><span class="name_mobile name_event_' +
                                data.result[k].id +
                                '" name="' +
                                data.result[k].eqLogic_id +
                                '" >' +
                                data.result[k].name +
                                '</span> <a  class="delete" name="' +
                                data.result[k].eqLogic_id +
                                '"  alt="' +
                                data.result[k].id +
                                '"><i style="font-size : 0.9em;padding-top:4px;color:rgb(155, 75, 70)" class="fas fa-minus-circle pull-right"></i></a><a  class="edit" name="' +
                                data.result[k].eqLogic_id +
                                '"  alt="' +
                                data.result[k].id +
                                '"><i style="font-size : 0.9em;padding-top:4px;color:rgb(58,90,85)" class="fas fa-sticky-note pull-right"></i></a><a  class="time_edit" name="' +
                                data.result[k].eqLogic_id +
                                '"  alt="' +
                                data.result[k].id +
                                '"><i style="font-size : 0.9em;padding-top:4px;color:' +
                                time_class +
                                '" class="fas fa-info-circle pull-right"></i></a> </li>';
                        } else {
                            autoComplete.push(data.result[k].name);
                        }
                    }
                }
                $('.todo[data-eqLogic_id="' + _id + '"] .list-group')
                    .empty()
                    .append(html);
                $('.todo[data-eqLogic_id="' + _id + '"] .list-group :checkbox')
                    .unbind()
                    .change(function () {
                        id = $(this).val();
                        if (this.checked) {
                ...
                $('.todo[data-eqLogic_id="' + _id + '"] .btn_add')
                    .unbind()
                    .on('click', function () {
                        id = $(this).val();
                        input = $('#' + id).val();
...
function addCmdToTable(_cmd) {
    ...
    if (
        _cmd.logicalId == 'getlist' ||
        _cmd.logicalId == 'new' ||
        _cmd.logicalId == 'list' ||
        _cmd.logicalId == 'removeall' ||
        _cmd.logicalId == 'refresh' ||
        _cmd.logicalId == 'cost' ||
        _cmd.logicalId == 'globalcost'
    ) {
        var tr = '<tr class="cmd" data-cmd_id="' + init(_cmd.id) + '" >';
        tr +=
            '<td><input class="cmdAttr form-control input-sm" data-l1key="id" style="display : none;"><input class="cmdAttr form-control input-sm" data-l1key="name" style="width : 90%;margin-left:auto;margin-right:auto;" placeholder="{{Nom}}" /></td>';
        tr +=
            '<td><span><input type="checkbox" data-size="mini" data-label-text="{{Visible}}" class="cmdAttr checkbox-inline" data-l1key="isVisible" checked/></span>';
        tr +=
            '<span class="type" type="info" style="display : none;">' +
